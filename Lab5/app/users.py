...
CREATE_USER_FIELDS = ['login', 'password', 'last_name', 'first_name', 'mid_name', 'role_id']
...
def create():
    ...
    if request.method == 'POST':
        ...
        pass_check_res, pass_check_msg = pass_checker.check_pass(user["password"])

        if not pass_check_res:
            flash(pass_check_msg, category='warning')
